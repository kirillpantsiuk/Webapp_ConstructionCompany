...
const DrawingWrapper = styled.div` width: 100%; height: 320px; background: #0f172a; border-radius: 15px; overflow: hidden; border: 1px solid rgba(56, 189, 248, 0.4); display: flex; align-items: center; justify-content: center; img { width: 100%; height: 100%; object-fit: contain; } `;
...
const TechnicalDashboard = () => {
  ...
  const [planData, setPlanData] = useState({
    ...
    steps: {
      ...
      s2: { active: true, desc: 'КРОК 02. РОЗМІТКА: Встановити осі будинку на місцевості.' },
  ...
  const handleObjectSelect = (id) => {
    ...
    if (inspection) {
      ...
      updatedSteps.s7.desc = `ЗДАЧА: Перевірити системи. Рекомендації координатора: ${inspection.recommendations || 'згідно норм ДБН'}.`;
  ...
  const finalizePlan = async () => {
    if (!planData.blueprintId || !planData.objectId) return setNotify({ open: true, message: 'Оберіть об’єкт та креслення!', severity: 'error' });
    const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
    const payload = {
    ...
    try {
      ...
      setNotify({ open: true, message: 'Технічний план успішно сформовано!', severity: 'success' });
  ...
  const handleDeleteTechPlan = async (id) => {
    ...
    try {
      ...
      setNotify({ open: true, message: 'Проєкт видалено!', severity: 'info' });
    } catch (err) {
      console.error(err);
  ...
  const printPlan = (plan) => {
    ...
    win.document.write(`
      <html><head><title>ПЛАН_${plan._id}</title><style>
        ...
        .step-card { margin-bottom: 25px; padding: 20px; border-left: 6px solid #38bdf8; background: #fafafa; border-radius: 0 10px 10px 0; }
        ...
        <div class="header">
          <div><h1>Технічний план будівництва</h1><p style="margin:0; font-weight:700; color:#64748b;">BUILD CRM SYSTEM</p></div>
        ...
        <div class="info-grid">
          ...
          <div class="info-item"><b>Тип ґрунту (Акт огляду)</b>${insp?.soilType || 'Не вказано'}</div>
          <div class="info-item"><b>Статус електрики</b>${insp?.electricity?.status || '—'}</div>
        </div>
        <div class="blueprint-box">
          <h2 style="font-size:14px; color:#0f172a; margin-top:0;">ПРИЙНЯТЕ ПРОЄКТНЕ РІШЕННЯ: ${blueprint?.name || 'Креслення №1'}</h2>
          <img src="http://localhost:5000${blueprint?.imageUrl}" alt="blueprint"/>
        </div>
        <h2 style="text-transform:uppercase; font-size:16px; border-bottom:2px solid #0f172a; padding-bottom:5px; margin-bottom:20px;">Покроковий графік робіт</h2>
        ...
        <div class="footer">
          <div class="sign"><div class="sign-line"></div>Координатор: ${userInfo?.login}</div>
  ...
  const handleSubmitInspection = async (e) => {
    ...
    try {
      ...
      setNotify({ open: true, message: 'Успішно збережено', severity: 'success' });
    } catch { setNotify({ open: true, message: 'Помилка валідації', severity: 'error' }); }
  };
  ...
  // Фільтрація матеріалів для етапу (Stage 4, 5, 6)
            ...
            {activeTab === 'inspections' && (
              <TableContainer>
                <StyledTable>
                  ...
                  <tbody>{filteredInspections.map(ins => { const obj = buildingObjects.find(o => o._id === (ins.objectId?._id || ins.objectId)); return (<tr key={ins._id}><td><b>{obj?.address || '—'}</b></td><td>{ins.soilType}</td><td>{ins.electricity?.status}</td><td><StatusBadge $active={ins.truckAccess}>{ins.truckAccess ? <CheckCircle2 size={14} /> : <XCircle size={14} />}{ins.truckAccess ? 'Так' : 'Ні'}</StatusBadge></td><td style={{textAlign:'right'}}><IconButton onClick={() => handlePrintInspection(ins)} style={{color:'#38bdf8'}} title="Друк акту"><Printer size={18}/></IconButton><IconButton style={{color:'#fbbf24'}} onClick={() => {setInspectionData(ins); setEditingId(ins._id); setOpenInspectionForm(true);}} title="Редагувати"><Edit size={18}/></IconButton><IconButton onClick={() => handleDeleteInspection(ins._id)} style={{color:'#ef4444'}} title="Видалити"><Trash2 size={18}/></IconButton></td></tr>) })}</tbody>
                </StyledTable>
              </TableContainer>
            )}
      ...
      <Dialog open={openInspectionForm} onClose={() => setOpenInspectionForm(false)} maxWidth="lg" fullWidth PaperProps={{style:{backgroundColor:'#1e293b', color:'white', borderRadius:'24px'}}}>
        <DialogContent>
          <FormGrid onSubmit={handleSubmitInspection} id="insp-form">
            ...
            <InputGroup $span={1.5}><label>Споруди на ділянці</label><textarea required value={inspectionData.existingStructures} onChange={e => setInspectionData({...inspectionData, existingStructures: e.target.value})} /></InputGroup>
